...
compute_build_fingerprints() {
    ...
    local input_file
    # Stage 10 controls source/cache orchestration. Its implementation must not
    # invalidate a compiled kernel cache when the pinned source and all actual
    # kernel-affecting inputs are unchanged. Keep the current Stage-10 hash as a
    # cache-ABI token so this change is backward-compatible with the cache built
    # from 10-prepare-host.sh SHA-256 7d5e521f... .
    local kernel_prepare_cache_abi_sha256="7d5e521f863bd61367490137d38f09c048c46bf6483feaa8fe73420274eccf43"
    ...
    kernel_base_fingerprint="$({
        ...
        for input_file in "${kernel_input_files[@]}"; do
            ...
            if [[ "$input_file" == "$SCRIPT_DIR/10-prepare-host.sh" ]]; then
                # Preserve the exact fingerprint stream generated by the current
                # Stage-10 implementation, but do not couple future orchestration
                # edits to kernel binary cache invalidation.
                printf '%s  %s\n' "$kernel_prepare_cache_abi_sha256" "$input_file"
            else
